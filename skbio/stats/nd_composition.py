...
def _composition_check(mat: Array, axis: int=-1):
    ...
    axis : int, default -1
        The axis along which to check the composition. Default is -1, which
        checks the last axis.
    """
    xp = aac.array_namespace(mat)
    if xp.any(mat < 0):
        raise ValueError("Cannot have negative proportions")
    if xp.sum(xp.all(mat == 0, axis=axis)) > 0:
        raise ValueError("Input matrix cannot have rows with all zeros")
    if mat.shape[axis]==1:
        postive_axis = (mat.ndim + axis) if axis<0 else axis
        raise ValueError(f'the dimension {postive_axis} of the input matrix is singleton')

def closure(mat: Array, axis: int=-1):
    ...
    if not aac.is_array_api_obj(mat):
        # otherwise assume the input is an array_like
        mat = np.array(mat)
    xp = aac.array_namespace(mat)
    if xp.any(mat < 0):
        raise ValueError("Cannot have negative proportions")
    if mat.ndim > 2:
        ...
        # if the input is a vector, we need to reshape it
        # maybe not necessary for nd case
        mat = mat.reshape(1, -1)
    if xp.any(xp.all(mat == 0, axis=axis)):
        raise ValueError("Input matrix cannot have rows with all zeros")
    mat = mat / xp.sum(mat, axis=axis, keepdims=True)
...
def clr(mat: Array,
    ...
    mat : array_like of shape (n_compositions, n_components)
        A matrix of proportions.
    axis : int, default -1
        The axis along which to perform the clr transformation.
    ...
    if not aac.is_array_api_obj(mat):
        # otherwise assume the input is an array_like
        # NOTE sam: np.array or np.asarray?
        mat = np.asarray(mat)
    xp = aac.array_namespace(mat)
    ...
    lmat = xp.log(mat)
    return lmat-xp.mean(lmat, axis=axis, keepdims=True)
...
def clr_inv(mat: Array,
    ...
    axis : int, default -1
        The axis along which to perform the inverse clr transformation.
    validate : bool, default True
    ...
    # for numerical stability, shitting the pre-exp values < 1
...
def ilr(mat:Array,
    ...
    basis : ndarray or sparse matrix, optional
        ...
        orthonormal basis.
    axis : int, default -1
        The axis along which to perform the ilr transformation.
    validate : bool, default True
    ...
    if basis is None:
        ...
        # check whether the basis is a 2D matrix
        if basis.ndim > 2:
            raise ValueError(
                "Basis needs to be a 2D matrix, not a %dD matrix." % basis.ndim)
        # check whether the basis has the right number of columns
        if basis.shape[1] != mat.shape[axis]:
            raise ValueError(
                "Basis needs to have %d columns, not %d." %
                (mat.shape[axis], basis.shape[1]))
        _check_orthogonality(basis)
    
    basis = xp.asarray(basis,
                    device=mat.device,
                    dtype=mat.dtype)
    # the mat is already validated, so skip the validation in clr()
    mat = clr(mat, axis=axis, validate=False)
    
    # when axis is not the last axis, permute the dimensions
    if axis != - 1:
        positive_axis = (mat.ndim + axis) if axis<0 else axis
        permute_order = tuple(i for i in range(mat.ndim)\
                            if i != positive_axis) + (positive_axis,)
        # permute_dims from array_api_standard uses the axes
        mat = xp.permute_dims(mat, axes=permute_order)
        return xp.permute_dims(mat @ basis.T, axes=permute_order)
    else:
        return mat @ basis.T


def ilr_inv(mat: Array,
            axis: int=-1,
            basis: Optional[Array]=None,
            validate: bool=True) -> Array:
    ...
    basis : ndarray or sparse matrix, optional
        ...
        orthonormal basis.
    axis : int, default -1
        The axis along which to perform the inverse ilr transformation.
    validate : bool, default True
    ...
    if axis!=-1:
        ...
        # permute_dims from array_api_standard uses the axes
        mat = xp.permute_dims(mat, axes=permute_order)
        return xp.permute_dims(mat @ basis, axes=permute_order)
    return clr_inv(mat @ basis, validate=validate)


def alr(mat:Array,
        denominator_idx:int=0,
        axis:int=-1,
        validate:bool=True):
    ...
    axis : int, default -1
        The axis along which to perform the alr transformation.
    validate : bool, default True
    ...
    if validate:
        if mat.shape[axis] < 2:
            positive_axis = (mat.ndim + axis) if axis<0 else axis
            raise ValueError(
                f"dimension D{positive_axis} of the input matrix is singleton"
            )
        if xp.any(mat <= 0):
            raise ValueError("Input matrix must be positive")
        if denominator_idx<-mat.shape[axis] or \
                            denominator_idx>=mat.shape[axis]:
            raise ValueError("denominator_idx is out of the range")
    pos_denominator_idx = (denominator_idx+mat.shape[axis]) if denominator_idx<0\
                                    else denominator_idx

    # no matter (n,) or (n, w, z), it will return n
    N = mat.shape[axis]
    numerator_indexs = tuple(i for i in range(N) if i != pos_denominator_idx)
    numerator_matrix = mat[..., numerator_indexs]
    # [denominator_idx] will keep the dimension, later automate broadcasting
    denominator_vector = mat[..., [denominator_idx]]
    return xp.log(numerator_matrix)-xp.log(denominator_vector)


def alr_inv(mat: Array,
            denominator_idx: int = 0,
            axis: int = -1):
    ...
    axis : int, default -1
        The axis along which to perform the alr inverse transformation.
    denominator_idx : int, default 0
        The index of the column (2-D matrix) or position (vector) of ``mat``
        where additional values will be inserted for the nature that inverse mapping
        from (D-1) dimensions to D dimensions . Default is 0 which
        specifies the first column or position.
    ...
    if axis!= -1:
        # permute the axis to the last dimension
        pos_axis = axis+mat.ndim if axis<0 else aixs
        permute_order = tuple(i for i in range(mat.ndim) \
                                    if i != pos_axis) + (pos_axis,)
        mat = xp.permute_dims(mat, axis=permute_order)
    N = mat.shape[-1]+1
    comp = xp.ones(mat.shape[:-1]+ (N,),
                   dtype=mat.dtype,
                   device=mat.device)
    pos_denominator_idx = denominator_idx if denominator_idx>=0 \
                                            else (denominator_idx+N)
    numerator_indexs = tuple(i for i in range(N) \
                                        if i != pos_denominator_idx)
    ...
    if axis!= -1:
        # recover the original permutation_order
        comp = xp.permute_dims(comp, axis=permute_order)
